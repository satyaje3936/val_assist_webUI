...
import extract_line_items as t
import fuse_equation_evaluator_poc as fuse_decoder
file_path = r"C:\PythonSv\streamlit\val_assist_webUI\Tools\fuse_files" 
output_path = r"C:\PythonSv\streamlit\val_assist_webUI\Tools\fuse_report"
...
def read_equations_cs_file_extract(file_path):
    ...
    cs_file_path = os.path.join(file_path,"CPU_GRANITE_RAPIDS_COMPUTE_DIE_M_Rule.cs")
    print(cs_file_path)
    ...
    content_no_multiline_comments = re.sub(r'/\*.*?\*/', '', content, flags=re.DOTALL)
    # Remove single-line comments (// ...)
    content_no_comments = re.sub(r'//.*', '', content_no_multiline_comments)

    # Pattern for LIRAMappingWrapper with possible multiple lineItem references in the dictionary
    lira_pattern = re.compile(
        r'LIRAMappingWrapper\(\s*lineItem\.([\w\d_]+)\.Value\s*,\s*(Fuses\.[^,]+)\s*,\s*new Dictionary<string, string>\s*{(.*?)}\s*\);',
        re.DOTALL
    )

    print(f"LIRA Pattern: {lira_pattern}")
    # Pattern for direct fuse assignments
    fuse_equation_pattern = re.compile(
        r'(Fuses\.[\w\.]+)\.Fuse(SetValue|BinaryValue)\s*=\s*(.*?);',
        re.DOTALL
    )
    ...
    for i, match in enumerate(lira_pattern.finditer(content_no_comments)):                             #Iterates over all matches of the lira_pattern in the file content.
        print(f"index:{i}, match_lira:{match} \n")
    ...
    for i, match in enumerate(fuse_equation_pattern.finditer(content_no_comments)):                  #Iterates over all matches of the fuse_equation_pattern in the file content
        print(f"index:{i}, match_fuse:{match} \n")
...
def export_cs_file_extract_to_excel(equation_results, output_path):
    ...
    output_file = os.path.join(output_path, 'fuse_extraction_results_with_CS_file.xlsx')
...
def map_json_with_cs(lira_json, equation_results, output_path):    
    ...
    # Check if equation_results is a pandas DataFrame (from Excel) or list of dictionaries
    if isinstance(equation_results, pd.DataFrame):
        # Handle DataFrame from Excel file
        for index, row in equation_results.iterrows():
            lineitem_attributes = str(row.get('LineItem_Attributes', ''))
            
            # Clean up the lineitem_attributes value and handle various "empty" cases
            lineitem_attributes = lineitem_attributes.strip()
            
            # Check if lineitem_attributes is empty, 'NA', 'nan', or other null-like values
            if (lineitem_attributes and 
                lineitem_attributes not in ['NA', 'nan', 'None', 'null', ''] and
                not pd.isna(row.get('LineItem_Attributes'))):
                
                lira_attrs = [attr.strip() for attr in lineitem_attributes.split(',')]
                
                # Get LIRA values for each attribute
                lira_values = []
                for attr in lira_attrs:
                    # Case-insensitive lookup
                    value = None
                    for json_attr, json_value in attribute_data_collections.items():
                        if attr.lower() == json_attr.lower():
                            value = json_value
                            break
                    lira_values.append(str(value) if value is not None else 'Not Found')
                
                lira_value_str = ', '.join([f"{attr}: {val}" for attr, val in zip(lira_attrs, lira_values)])
            else:
                # Handle cases where there are no LineItem attributes
                lineitem_attributes = 'NA'  # Ensure consistent 'NA' value
                lira_value_str = 'NA'
            
            # Create new row with added LIRA attribute values - preserving all original columns
            mapped_row = row.to_dict()
            mapped_row['LineItem_Attributes'] = lineitem_attributes  # Ensure 'NA' is preserved
            mapped_row['LIRA_Attribute_Values'] = lira_value_str
            
            # Ensure Original_Matches is preserved and handle empty/null values
            original_matches = row.get('Original_Matches', '')
            if pd.isna(original_matches) or str(original_matches).strip() in ['', 'nan', 'None', 'null']:
                mapped_row['Original_Matches'] = 'None'
            else:
                mapped_row['Original_Matches'] = str(original_matches).strip()
            
            # Ensure Duplicates_Removed is preserved and handle empty/null values
            duplicates_removed = row.get('Duplicates_Removed', '')
            if pd.isna(duplicates_removed) or str(duplicates_removed).strip() in ['', 'nan', 'None', 'null']:
                mapped_row['Duplicates_Removed'] = 'No'
            else:
                mapped_row['Duplicates_Removed'] = str(duplicates_removed).strip()
                
            mapped_result.append(mapped_row)
    
    else:
        # Handle original list of dictionaries format
...
def export_map_json_cs_file_extract_to_excel(mapped_result, output_path):
    ...
    # Reorder columns to place LIRA_Attribute_Values next to LineItem_Attributes for better readability
    if 'LineItem_Attributes' in df.columns and 'LIRA_Attribute_Values' in df.columns:
        # Get the current column order
        cols = df.columns.tolist()
        
        # Find the index of LineItem_Attributes
        lineitem_idx = cols.index('LineItem_Attributes')
        
        # Remove LIRA_Attribute_Values from its current position
        cols.remove('LIRA_Attribute_Values')
        
        # Insert LIRA_Attribute_Values right after LineItem_Attributes
        cols.insert(lineitem_idx + 1, 'LIRA_Attribute_Values')
    ...
    output_file = os.path.join(output_path, f'fuse_extraction_results_with_LIRA_attr_and_values_{timestamp}.xlsx')
    
    # Export to Excel with formatting
    with pd.ExcelWriter(output_file, engine='xlsxwriter') as writer:
        df.to_excel(writer, sheet_name='Enhanced_Results', index=False)
        
        # Get workbook and worksheet for formatting
        workbook = writer.book
        worksheet = writer.sheets['Enhanced_Results']
        ...
        if 'Line_Number' in df.columns:  # Excel data structure
            ...
            worksheet.set_column('E:E', 100) # LIRA_Attribute_Values (moved next to LineItem_Attributes)
            worksheet.set_column('F:F', 15)  # Attribute_Count
            worksheet.set_column('G:G', 30)  # Assigned_Value
            worksheet.set_column('H:H', 150) # Complete_Code_Line
            worksheet.set_column('I:I', 60)  # Original_Matches
            worksheet.set_column('J:J', 15)  # Duplicates_Removed
        else:  # Original data structure
            worksheet.set_column('A:A', 20)  # Source
            worksheet.set_column('B:B', 50)  # LIRA Attribute
            worksheet.set_column('C:C', 80)  # LIRA Value
            worksheet.set_column('D:D', 60)  # Fuse Name
            worksheet.set_column('E:E', 100) # Equation
        
        # Set row height for better readability
        worksheet.set_default_row(25)
    
    print(f"Results with LIRA values exported to: {output_file}")
    return output_file

def load_excel_from_extract_line_items(excel_file_path):
    """
    Load Excel file generated by extract_line_items.py
    """
    try:
        df = pd.read_excel(excel_file_path, sheet_name='All_Occurrences')
...
def map_fuse_report_with_cs(fuse_data, enhanced_excel_path, sku):   
    """
    Map fuse data with Excel file from enhanced results and add Fuse_Value column
    
    Args:
        fuse_data: Dictionary containing fuse information with values and comments
        enhanced_excel_path: Path to the enhanced Excel file with LIRA attributes
        sku: SKU identifier
    
    Returns:
        List of mapped fuse results with added Fuse_Value column
    """
    mapped_fuses = []
    
    # Load the enhanced Excel file
    try:
        df = pd.read_excel(enhanced_excel_path, sheet_name='Enhanced_Results')
        print(f"✅ Loaded enhanced Excel file: {enhanced_excel_path}")
        print(f"📊 Processing {len(df)} rows from enhanced Excel")
    except Exception as e:
        print(f"❌ Error loading enhanced Excel file: {e}")
        return []

    # Process each row from the Excel file
    for index, row in df.iterrows():
        # Extract fuse name and clean it
        fuse_name_raw = str(row.get("Fuse_Name", "")).strip()
        fuse_name = fuse_name_raw.split('.')[-1].lower() if fuse_name_raw else ""
        
        # Get fuse value from fuse_data
        fuse_value = "Not found"
        actual_fuse_value = "Not found"
        comment = "Not found"
        
        if fuse_name and fuse_name in fuse_data:
            fuse_info = fuse_data[fuse_name]
            if isinstance(fuse_info, dict):
                fuse_value = fuse_info.get("value", "Not found")
                actual_fuse_value = fuse_value  # Same as fuse_value for consistency
                comment = fuse_info.get("comment", "Not found")
                if not comment or str(comment).strip() == "":
                    comment = "Not found"
            else:
                fuse_value = str(fuse_info)
                actual_fuse_value = fuse_value
        else:
            if fuse_name:
                print(f"⚠️ Fuse '{fuse_name}' not found in fuse report data")
        
        # Get LineItem_Attributes and LIRA_Attribute_Values with proper NA handling
        lineitem_attributes = row.get("LineItem_Attributes", "")
        lira_attribute_values = row.get("LIRA_Attribute_Values", "")
        
        # Handle LineItem_Attributes: convert blank, null, or nan values to "NA"
        if (pd.isna(lineitem_attributes) or 
            lineitem_attributes in ['', 'nan', 'None', 'null'] or 
            str(lineitem_attributes).strip() == ''):
            lineitem_attributes = "NA"
        
        # Handle LIRA_Attribute_Values: convert blank, null, or nan values to "NA"
        if (pd.isna(lira_attribute_values) or 
            lira_attribute_values in ['', 'nan', 'None', 'null'] or 
            str(lira_attribute_values).strip() == ''):
            lira_attribute_values = "NA"
        
        # Create mapped result with Fuse_Value column next to Fuse_Name
        mapped_entry = {
            "Line_Number": row.get("Line_Number", ""),
            "Fuse_Type": row.get("Fuse_Type", ""),
            "Fuse_Name": fuse_name_raw,
            "Actual_Value_from_Report": actual_fuse_value,
            #"Fuse_Value": fuse_value,  # New column added next to Fuse_Name
            "LIRA Attributes": lineitem_attributes,  # Renamed from LineItem_Attributes
            "LIRA Value": lira_attribute_values,     # Renamed from LIRA_Attribute_Values
            "Attribute_Count": row.get("Attribute_Count", ""),
            "Assigned_Value": row.get("Assigned_Value", ""),
            "Fuse_Equation": row.get("Complete_Code_Line", ""),  # Renamed from Complete_Code_Line
            #"Original_Matches": row.get("Original_Matches", ""),
            #"Duplicates_Removed": row.get("Duplicates_Removed", ""),
            "HSD_Info": comment
        }
        
        mapped_fuses.append(mapped_entry)
            
    print(f"✅ Successfully mapped {len(mapped_fuses)} fuse entries with fuse values")
    return mapped_fuses
    
def export_fuse_mapping_to_excel(fuse_mapped_result, output_path, sku):
    """
    Export fuse mapping results to Excel with enhanced formatting and column layout
    """
    df = pd.DataFrame(fuse_mapped_result)
    timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
    output_file = os.path.join(output_path, f'{sku}_fuse_extraction_results_with_fuse_values_{timestamp}_final.xlsx')
    
    # Export to Excel with formatting
    with pd.ExcelWriter(output_file, engine='xlsxwriter') as writer:
        df.to_excel(writer, sheet_name='Fuse_Mapping_Results', index=False)
        
        # Get workbook and worksheet for formatting
        workbook = writer.book
        worksheet = writer.sheets['Fuse_Mapping_Results']
        ...
        worksheet.set_column('B:B', 15)   # Fuse_Type
        worksheet.set_column('C:C', 50)   # Fuse_Name
        worksheet.set_column('D:D', 20)   # Fuse_Value (new column)
        worksheet.set_column('E:E', 80)   # LIRA Attributes (renamed from LineItem_Attributes)
        worksheet.set_column('F:F', 100)  # LIRA Value (renamed from LIRA_Attribute_Values)
        worksheet.set_column('G:G', 15)   # Attribute_Count
        worksheet.set_column('H:H', 30)   # Assigned_Value
        worksheet.set_column('I:I', 150)  # Fuse_Equation (renamed from Complete_Code_Line)
        worksheet.set_column('J:J', 60)   # Original_Matches
        worksheet.set_column('K:K', 15)   # Duplicates_Removed
        worksheet.set_column('L:L', 30)   # Actual_Value_from_Report
        worksheet.set_column('M:M', 60)   # HSD_Info
    ...
    print(f"✅ Fuse mapping results with fuse values exported to: {output_file}")
    return output_file
...
def main():
    """
    Main function with command-line argument support for controlling specific file creation.
    
    Usage examples:
    python fccb_report_dump.py --cs_file                    # Only create Excel file from CS extraction
    python fccb_report_dump.py --lira_value                 # Only create enhanced Excel with LIRA values
    python fccb_report_dump.py --cs_file --lira_value       # Create both files
    python fccb_report_dump.py                              # Default: create all files (full pipeline)
    """
    parser = argparse.ArgumentParser(description='FCCB Report Dump with selective file creation options')
    parser.add_argument('--cs_file', action='store_true', 
                       help='Create Excel file after extracting from CS file only')
    parser.add_argument('--lira_value', action='store_true', 
                       help='Create enhanced Excel file with LIRA values populated only')
    parser.add_argument('--sku', type=str, default='Q7YT', 
                       help='SKU name (default: Q7YT)')
    parser.add_argument('--full_pipeline', action='store_true', 
                       help='Run the complete pipeline (all files)')
    
    args = parser.parse_args()
    
    print("In the main function.")
    print(f"Command-line options: cs_file={args.cs_file}, lira_value={args.lira_value}, full_pipeline={args.full_pipeline}")
    
    # If no specific option is provided, run full pipeline
    if not args.cs_file and not args.lira_value and not args.full_pipeline:
        args.full_pipeline = True
        print("No specific option provided, running full pipeline by default")
    ...
    # Step 1: CS File extraction (if requested or full pipeline)
    if args.cs_file or args.full_pipeline:
        print("\n🔧 Creating Excel file from CS file extraction...")
        excel_file_path = t.extract_info_from_cs_file()  # This returns the path to the Excel file
        print(f"\n✅ Extract line items completed: {excel_file_path}")
        
        if not excel_file_path or not os.path.exists(excel_file_path):
            print("❌ CS file extraction failed or Excel file not found")
            if args.cs_file and not args.lira_value and not args.full_pipeline:
                return None
    
    # Step 2: LIRA Value mapping (if requested or full pipeline)
    if args.lira_value or args.full_pipeline:
        print("\n🔧 Creating enhanced Excel file with LIRA values...")
        
        # If we haven't created the CS file yet, we need it for LIRA mapping
        if not excel_file_path:
            print("📋 CS file extraction needed for LIRA value mapping...")
            excel_file_path = t.extract_info_from_cs_file()
            print(f"✅ Extract line items completed: {excel_file_path}")
        
        if excel_file_path and os.path.exists(excel_file_path):
            df_equation_results = load_excel_from_extract_line_items(excel_file_path)
            
            if df_equation_results is not None:
                print(f"\n✅ Excel file loaded successfully")
                print(f"📊 Processing {len(df_equation_results)} rows")
                
                # Map JSON with Excel data and add LIRA attribute values
                mapped_result = map_json_with_cs(lira_json, df_equation_results, output_path)
                print(f"\n✅ LIRA attribute values mapped successfully")
                
                # Export enhanced results to Excel
                enhanced_excel_path = export_map_json_cs_file_extract_to_excel(mapped_result, output_path)
                print(f"\n✅ Enhanced Excel file created: {enhanced_excel_path}")
            else:
                print("❌ Failed to load Excel file for LIRA value mapping")
                if args.lira_value and not args.cs_file and not args.full_pipeline:
                    return None
        else:
            print("❌ Excel file not found for LIRA value mapping")
            if args.lira_value and not args.cs_file and not args.full_pipeline:
                return None
    
    # Step 3: Full pipeline completion (only if full_pipeline is requested)
    if args.full_pipeline:
        print("\n🔧 Completing full pipeline with fuse mapping...")
        
        if enhanced_excel_path:
            fuse_mapped_result = map_fuse_report_with_cs(fuse_dict, enhanced_excel_path, sku)
            output_file = export_fuse_mapping_to_excel(fuse_mapped_result, output_path, sku)
            print(f"\n✅ Full pipeline completed. Final output: {output_file}")
        else:
            print("❌ Enhanced Excel file not available for final fuse mapping")
            return None
    ...
    if args.cs_file or args.full_pipeline:
        if excel_file_path:
            print(f"✅ CS File Excel: {excel_file_path}")
        else:
            print("❌ CS File Excel: Failed to create")
    
    if args.lira_value or args.full_pipeline:
        if enhanced_excel_path:
            print(f"✅ Enhanced Excel with LIRA values: {enhanced_excel_path}")
        else:
            print("❌ Enhanced Excel with LIRA values: Failed to create")
    
    if args.full_pipeline:
        print(f"✅ Final fuse mapping output: {output_file if 'output_file' in locals() else 'Failed to create'}")
